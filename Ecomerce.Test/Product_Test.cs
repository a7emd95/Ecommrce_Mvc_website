...
namespace Ecomerce.Test
{
    public class Product_Test
    {
        ...
        {
            var result = productAppService.GetProductByID(5);
            Assert.That(result.Name, Is.EqualTo("IMac"));
        }
        ...
        {
            var result = productAppService.GetProductByID(1);
            Assert.That(result.Quentity, Is.EqualTo(4));
        }
        ...
        {
            var product = new ProductViewModel() { Name = "apple LabTop", Price = 120000, Quentity = 8, Description = "Apple product",
                Image = "1.jbg", Category_Id = 1 };
